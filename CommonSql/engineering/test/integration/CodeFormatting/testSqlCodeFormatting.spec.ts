...
import { codeFormattingTestCases } from "./sqlCodeFormattingTestCases";
import { testGrammarRuleNames } from "@engineering/common/SqlFakedConfig";

describe("test SQL CodeFormatting", () => {
    ...
    for (let i = 0; i < codeFormattingTestCases.length; i++) {
        ...
        it(`should format script ${i} "${script}" correctly`, () => {
            ...
            expect(result.replace(/\s+/g, " ") === testCase.expected.replace(/\s+/g, " ")).toBe(true);
        });
        progressIndex++;
