...
export class MockFoldingProvider {
    ...
    async provideFoldingRanges(model: editor.ITextModel, options: languages.FoldingContext, token: CancellationToken): Promise<any> {
        const results = (await MockLanguageServiceFacade.GetResultFromPipeline({
            code: model.getValue(),
            reason: LanguageServiceFeature.CodeFolding,
        } as ILanguageServiceRequest, this.languageServiceConfig)) as languages.FoldingRange[];
